    ...
    <footer className="border-t border-ink-200 bg-ink-50">
      <div className="container-x py-14">
        <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-[1.4fr_1fr_1fr]">
          <div>
            <Logo />
            <p className="mt-4 max-w-xs text-sm leading-relaxed text-ink-500">{FOOTER.tagline}</p>
          </div>
          ...
          <nav aria-label="Хөлийн цэс">
            ...
            <ul className="mt-4 space-y-2.5">
              {NAV_LINKS.map((l) => (
                <li key={l.href}>
                  <Link href={l.href} className="text-sm text-ink-600 transition hover:text-ink-900">
                    {l.label}
                  </Link>
                </li>
              ))}
            ...
            <ul className="mt-4 space-y-2.5">
              {FOOTER.socials.map((s) => (
                <li key={s.label}>
                  <a
                    href={s.href}
                    className="inline-flex items-center gap-2 text-sm text-ink-600 transition hover:text-ink-900"
        ...
        <div className="mt-12 flex flex-col items-start justify-between gap-4 border-t border-ink-200 pt-6 sm:flex-row sm:items-center">
          <p className="text-xs text-ink-500">{FOOTER.copyright}</p>
