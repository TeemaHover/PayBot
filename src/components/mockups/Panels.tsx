...
export function OrderPanel({ className }: { className?: string }) {
  return (
    <div className={clsx('rounded-2xl border border-ink-600 bg-ink-900 p-5', className)}>
      <div className="flex items-center justify-between">
        <div>
          <p className="text-[11px] uppercase tracking-wider text-white/40">Захиалга</p>
          <p className="text-sm font-semibold">#PB-10428</p>
        </div>
        <span className="flex items-center gap-1.5 rounded-full border border-amber-400/30 bg-amber-400/10 px-2.5 py-1 text-[11px] font-semibold text-amber-300">
      ...
      <div className="mt-4 flex items-center justify-between border-t border-ink-700 pt-4">
        <span className="text-sm text-white/45">Нийт дүн</span>
        <span className="text-xl font-bold text-brand-200">{mnt(178000)}</span>
      </div>
  ...
  return (
    <div className={clsx('rounded-2xl border border-ink-600 bg-ink-900 p-5', className)}>
      <div className="flex items-center gap-2">
        <span className="grid h-8 w-8 place-items-center rounded-lg bg-aqua-400/15 text-aqua-300">
          <Wallet className="h-4 w-4" />
        </span>
        <div>
          <p className="text-sm font-semibold">QPay төлбөр</p>
          <p className="text-[11px] text-white/40">Захиалга #PB-10428</p>
        </div>
      </div>
      ...
      <div className="mt-5 flex items-center gap-5">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-[11px] uppercase tracking-wider text-white/40">Төлөх дүн</p>
  ...
  return (
    <div className={clsx('rounded-2xl border border-ink-600 bg-ink-900 p-5', className)}>
      <div className="flex items-center justify-between">
        ...
        <span className="chip rounded-full border border-ink-600 px-2.5 py-1 text-[11px] text-white/50">
          Сүүлийн 30 хоног
        </span>
      </div>
      ...
      <dl className="mt-4 grid grid-cols-2 gap-2.5">
        {stats.map((s) => (
          <div key={s.label} className="rounded-xl border border-ink-700 bg-ink-850 p-3">
            <dt className="text-[10px] uppercase tracking-wide text-white/40">{s.label}</dt>
            <dd className="mt-1 flex items-baseline gap-2">
              <span className="text-lg font-bold">{s.value}</span>
              <span className="text-[11px] font-semibold text-mint-400">{s.trend}</span>
            </dd>
          </div>
        ))}
      </dl>
      ...
      <div className="mt-4 rounded-xl border border-ink-700 bg-ink-850 p-3">
        <p className="text-[10px] uppercase tracking-wide text-white/40">Өдрийн захиалга</p>
      ...
      <div className="mt-4 space-y-2">
        {[
          ...
          <div
            key={id as string}
            className="flex items-center justify-between rounded-lg border border-ink-700 px-3 py-2 text-sm"
          >
            <span className="text-white/70">{id as string}</span>
            <span
              className={clsx(
                'flex items-center gap-1 text-xs font-semibold',
                paid ? 'text-mint-400' : 'text-amber-300',
              )}
            >
              {status as string}
...
function QrArt() {
  ...
  return (
    <div
      aria-hidden="true"
      className="grid h-24 w-24 shrink-0 grid-cols-7 gap-[3px] rounded-xl bg-white p-2"
    >
      {cells.map((on, i) => (
        <span key={i} className={clsx('rounded-[2px]', on ? 'bg-ink-950' : 'bg-transparent')} />
      ))}
    </div>
  );
}
